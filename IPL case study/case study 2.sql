...
SELECT
    ...
    (
        ...
        AND ipl_bidding_details.BIDDER_ID = ipl_bidder_details.BIDDER_ID
    ) /
    (
        SELECT no_of_bids
        FROM ipl_bidder_points
        WHERE ipl_bidder_points.bidder_id = ipl_bidder_details.BIDDER_ID
    ) * 100 AS 'Percentage of Wins(%)'
FROM
    ipl_bidder_details
ORDER BY 3 DESC;
...
WITH TeamBids AS (
    SELECT
        ipl_team.TEAM_NAME AS team_name,
        SUM(ipl_bidder_points.NO_OF_BIDS) AS total_bid
    FROM
        ipl_bidder_points
    INNER JOIN
        ipl_bidding_details ON ipl_bidder_points.BIDDER_ID = ipl_bidding_details.BIDDER_ID
    INNER JOIN
    ...
    INNER JOIN
        ipl_team ON ipl_match.TEAM_ID1 = ipl_team.TEAM_ID
    GROUP BY
        ipl_team.TEAM_NAME)
        SELECT
    team_name,
    total_bid
FROM TeamBids
WHERE total_bid = (SELECT MAX(total_bid) FROM TeamBids)
   OR total_bid = (SELECT MIN(total_bid) FROM TeamBids);


3. In a given stadium, what is the percentage of wins by a team which has won the toss?
SELECT
    ipl_stadium.STADIUM_id,
    COUNT(ipl_match.MATCH_ID) AS total_matches,
    SUM(CASE WHEN ipl_match.toss_winner = ipl_match.match_winner THEN 1 ELSE 0 END) AS toss_winning_matches,
    (SUM(CASE WHEN ipl_match.toss_winner = ipl_match.match_winner THEN 1 ELSE 0 END) / COUNT(*)) * 100 AS percentage_wins
FROM 
    ipl_match
INNER JOIN
    ipl_match_schedule ON ipl_match.MATCH_ID = ipl_match_schedule.MATCH_ID
INNER JOIN
    ipl_stadium ON ipl_match_schedule.STADIUM_ID = ipl_stadium.STADIUM_ID
GROUP BY
    ipl_stadium.STADIUM_id;
...
4. What is the total no. of bids placed on the team that has won the highest no. of matches?
    ...
    SELECT
        ipl_team.TEAM_NAME AS team_name,
        SUM(ipl_bidder_points.NO_OF_BIDS) AS total_bid,
        sum(ipl_match.MATCH_WINNER) as total_wins
    FROM
        ipl_bidder_points
    INNER JOIN
        ipl_bidding_details ON ipl_bidder_points.BIDDER_ID = ipl_bidding_details.BIDDER_ID
...
5. From the current team standings, if a bidder places a bid on which of the teams, there is a possibility of (s)he winning the highest no. of points – in simple words, 
